...
export function getIDSRCase ({user}) {
  let schoolModel = {
    id: uuid(),
    user: user.user,
...
export function mapGradeFromInfo (idsrCase) {
  let grades = []
  for (var i = 0; i < MAX_GRADE_NUMBER; i++) {
    grades.push({
      index: i,
...
export function mapFeeFromInfo (idsrCase) {
  let fees = []
  for (var i = 0; i < MAX_FEE_NUMBER; i++) {
    fees.push({
      index: i,
...
export function mapAttendanceFromInfo (idsrCase) {
  let attendances = []
...
export function getGradeInfo (idsrCase) {
  let gradeInfos = mapGradeFromInfo(idsrCase)
  let nextVisible = 0
...
export function getFeeInfo (idsrCase) {
  let feeInfos = mapFeeFromInfo(idsrCase)
  let nextVisible = 0
...
export function getAttendanceInfo (idsrCase) {
  let attendanceInfos = mapAttendanceFromInfo(idsrCase)
  let nextVisible = 0
...
export function getSISId (idsrCase) {
  const info = idsrCase.schoolInfo
  return `${info.docId} - ${idsrCase.regID} - ${idsrCase.created}`
}

export function getId (idsrCase) {
  return toIndexableString([
    idsrCase.regID,
    idsrCase.created,
    idsrCase.schoolInfo.docId
  ]).replace(/\u0000/g, '\u0001')
}

export function completeCase (idsrCase) {
  const id = idsrCase._id || getId(idsrCase)

  return Object.assign({_id: id}, idsrCase, {
    schoolInfo: Object.assign({sisId: getSISId(idsrCase)}, idsrCase.schoolInfo),
    complete: true,
    processEdit: true
  })
}
